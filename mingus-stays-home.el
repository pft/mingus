...
;; Copyright (C) 2006-2007 Niels Giesen <com dot gmail at nielsgiesen, in reversed
;; order>
...
;; Removed silly dependencies for calculating time of song (mpd already knows about
;; them: RTFM afore thou proggest pft!).
...
;; NOTE (at version 0.21) : Version 0.21 is actually the first published version of
;; mingus-stays-home. To provide easy checking whether the versions of mingus.el and
;; mingus-stays-home (should) work together, it was decided to give them concurrent
;; version numbers.
...
;; Make sure you have a version of mingus.el of the same version number available in
;; your load-path.
...
;;; version 0.22 required too much, glad to be able to trim it down again in version
;;; 0.23 after having read some documentation:
...
;;; cd. Actually, this can be customized with the variables `mingus-blank-string'
;;; and `mingus-burns-format-string'
...
;;; ( use dto's --already deprecated-- golisp-mode to make use of the indexing in this
;;; file: find it at http://dto.freeshell.org/notebook/GoLisp.html )
...
(defconst mingus-mpd-playlist-dir (expand-file-name (concat (mingus-get-config-option mingus-mpd-config-file "playlist_directory") "/")) "Root for mpd.")
...
(defun _mingus-playlist-get-filename-at-p ()
  (plist-get (car (get-text-property (point-at-bol) 'details)) 'file))
...
(defun _mingus-get-filename-from-playlist-relative ()
  "In buffer *Mingus*, retrieve filename of song at point, relative to `mingus-mpd-root'"
...
(defun _mingus-get-filename-from-browser-relative ()
  "In buffer *Mingus Browse*, retrieve filename of song at point, relative to `mingus-mpd-root'"
...
(defalias 'mingus-get-filename '_mingus-get-filename-absolute "Get absolute filename for song-at-p.")
...
Region is between (beginning of line of) BEG and (beginning of line of) END."
  ...
  (let ((beg (if mark-active (_mingus-bol-at beg) (point-at-bol)))
    ...
    (mapcar '_mingus-make-absolute-filename (split-string (buffer-substring-no-properties beg end) "\n" t))))
...
To be used for passing the right data to mpd in dired."
  (cons (cons mingus-mpd-root "")
	(mingus-make-alist-reversed
	 (remove 
          "" (split-string (shell-command-to-string (format "symlinks -crv %s" mingus-mpd-root))
                           (format "\\(\\(\n*relative:\\|\n*other_fs:\\|\n*dangling:\\| ->\\) \\|%s\\|\n\\)" 
                                   mingus-mpd-root))))))
...
Try again at a later time if this happens."
  ...
  (let
	...
	(when (y-or-n-p (format "Something has gone wrong.  Possibly mpd does not know about the file.
...
(defun mingus-dired-file ()
  ...
  (dired
   (cond
    ((mingus-directoryp) (_mingus-make-absolute-filename (cdr (mingus-directoryp)))) ;is it a directory?
    ((mingus-playlistp) mingus-mpd-playlist-dir)			     ;is it a playlist?
    (t (_mingus-get-parent-dir))) "-al"))				     ;it is a song!
...
(defun mingus-read-entire-metadata ()
  (mapcar (lambda (sublist) 
            (mapcar (lambda (item) 
                      (cond
                       ((eq item 'Pos) :pos)
                       ((symbolp item) (intern-soft (concat ":" (downcase (symbol-name item)))))
    ...
    (case type
      (mp3
       (shell-command (format "id3v2 %s %s"
			      ...
			      well-quoted-name)))
      (ogg
       (shell-command (format "vorbiscomment -w %s %s"
      			      (mapconcat #'(lambda (item) item)
      					 (mapcar (lambda (option)
      						   (concat "-t \"" (id3-item-vorbiscomment (mingus-id3-get-item option))
      							   "=" (apply mingus-id3-format (list (mingus-read-id3-elt option))) "\""))
      						 taglist) " ")
                              well-quoted-name)))
      (flac
       (shell-command (format "metaflac %s %s"
			      (mapconcat #'(lambda (item) item)
					 (mapcar (lambda (option)
						   (concat "--remove-tag="
...
ITEM must be one of the elements in the variable `*mingus-id3-items*'"
  (let* ((list (funcall (id3-item-list-or-list-function (mingus-id3-get-item item))))
	...
	(case (mingus-what-type (mingus-get-filename-for-shell))
	  (mp3 (number-to-string (or (cdr (assoc result (mingus-make-genre-alist))) 0)))
	  (t result))
      result)))
...
(defun mingus-burns-del ()
  ...
  (save-excursion
    (let* ((buffer-read-only nil)
	   ...
	   (min:secs (sec->min:sec length-of-song-at-p)))
